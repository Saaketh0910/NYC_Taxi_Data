...
def transform_ts_data_info_features(
    ...
    for location_id in location_ids:
        try:
            ...
            # Pad with zeroes at the beginning to ensure length is at least window_size
            if len(values) < window_size:
                padding_size = window_size - len(values)
                values = np.pad(values, (padding_size, 0), 'constant')  # Pad at the beginning
                times = np.pad(times, (padding_size, 0), constant_values=np.datetime64('NaT'))  # Pad times with NaT

            # Ensure there are enough rows to create at least one window
            if len(values) < window_size:  # Double check after padding
                raise ValueError("Not enough data to create even one window after padding.")
            ...
            for i in range(0, len(values) - window_size + 1, step_size):
                ...
                features = values[i : i + window_size]
                # Get the corresponding target timestamp
                target_time = times[i + window_size - 1]  # Use correct index for target timestamp
